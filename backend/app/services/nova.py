...
def _date(value: str | None, now: datetime) -> datetime.date | None:
    ...
    try:
        ...
        pass
    match = re.fullmatch(r"(?:next|this)\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)", value)
    if match:
...
def _matches(db: Session, query: str) -> list[Event]:
    words = [word for word in re.findall(r"[a-z0-9]+", query.lower()) if len(word) > 2 and word not in {"the", "my", "event", "appointment"}]
    events = db.query(Event).filter(Event.status != "cancelled").order_by(Event.start_time).all()
    exact = [event for event in events if event.title.lower() == query.strip().lower()]
    if exact:
        return exact
    return [event for event in events if words and all(word in event.title.lower() for word in words)]
...
def _free_slots_for_day(
    ...
    """Return available time gaps for a calendar day."""
...
def execute_intent(db: Session, intent: CalendarIntent, now: datetime | None = None) -> dict:
    ...
    if intent.intent == "UNKNOWN":
        return {"ok": False, "status": "unknown", "message": "I can view, create, reschedule, or delete calendar events. Please try phrasing that request again."}

    if intent.intent == "GET_EVENTS":
        day = _date(intent.date, now)
        if intent.date and not day:
            return {"ok": False, "status": "invalid_date", "message": "I couldn't understand that date. Please use a date such as tomorrow or next Monday."}
        if day:
            start, end = datetime.combine(day, time.min), datetime.combine(day + timedelta(days=1), time.min)
            events = db.query(Event).filter(Event.status != "cancelled", Event.start_time < end, Event.end_time > start).order_by(Event.start_time).all()
            label = f"{day:%A, %B} {day.day}"
        else:
            events = db.query(Event).filter(Event.status != "cancelled", Event.end_time >= now).order_by(Event.start_time).all()
            label = "upcoming"
        message = f"You have no events {('on ' + label) if day else label}." if not events else f"Here {'is' if len(events) == 1 else 'are'} your {label} event{'s' if len(events) != 1 else ''}."
        return {"ok": True, "status": "success", "message": message, "events": [_event_payload(event) for event in events]}

    if intent.intent == "GET_NEXT_EVENT":
        event = db.query(Event).filter(Event.status != "cancelled", Event.start_time >= now).order_by(Event.start_time).first()
        if not event:
            return {"ok": True, "status": "success", "message": "You have no upcoming events.", "events": []}
        return {"ok": True, "status": "success", "message": f"Your next event is {event.title} at {event.start_time.strftime('%I:%M %p').lstrip('0')} on {event.start_time:%A, %B} {event.start_time.day}.", "events": [_event_payload(event)]}
